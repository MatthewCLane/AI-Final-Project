'''
This file is to help us find the best filter for the AI

NOTE: This is the file that we edited to try and find the best Kernel, and PoolSize.
...
fileOutput = 'D:\\model saves conv\\info.txt'
...
# These variables are here so we can change them if we had to stop the loop at any point
counter = 9
counter2 = 1
poolsize = 1
iteration = 0
...
# This is what we want our filter to go up to for the first conv2d
while counter < 20:
    counter += 1
    counter2 = 1

    # This is what we want our filter to go up to for the second conv2d
    while counter2 < 30:
        counter2 += 1

        # These are here so if we need to stop the loop for some reason then we can start at a
        # specific variable.
        #if iteration2 == 0:
        #    counter2 = 18
        #    iteration2 += 1
        ...
        ###########################################################################################################
        #
        # This while loop was running at the start. Was trying to cover pool size with filter at the same time.
        # Not long after starting the loop, we realized that it wouldn't be even close to finish before the
        # project was due.
        #
    ...
    # https://www.geeksforgeeks.org/keras-fit-and-keras-fit_generator/
        ...
        model.save('D:\\model saves conv\\adamE12085.09L1N150L2N610_' + str(counter) + '_' + str(counter2) + 'K' +
                   '3,2' + '.h5')
    ...
    # Plot the first values
        ...
        accuracyString = 'D:\\model saves conv\\Accuracy\\adamE12085.09accuracyScores_L1N150L2N610_' + str(counter) + \
                         '_' + str(counter2) + 'K' + '3,2' + '.png'
    ...
    # Plot the second values
        ...
        accuracyString = 'D:\\model saves conv\\loss\\adamaxE12085.09lossScores_L1N150L2N610_' + str(counter) + '_' + str(counter2) + 'K' + \
                         '3,2' + '.png'
        plt.savefig(accuracyString, bbox_inches='tight')
    #plt.show()
    ...
    # Make the confusion matrix with the predictions vs the actual values
        ...
        # Write all of the important things to the file
    ...
    # Visualize the confusion matrix
        ...
        plt.ylabel('Actual')
        accuracyString = 'D:\\model saves conv\\Confusion Matrix\\adamE12085.09confusionMatrix_L1N150L2N610_' + str(counter) \
                         + '_' + str(counter2) + 'K' + '3,2' + '.png'
        plt.savefig(accuracyString, bbox_inches='tight')
    #plt.show()
        file.close()
